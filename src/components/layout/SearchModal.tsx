...
export function SearchModal({ isOpen, onClose }: SearchModalProps) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      setLoading(true);
      try {
        const res = await fetch(`/api/products/search?q=${encodeURIComponent(normalizedQuery)}`);
        ...
        <>
          ...
          >
            <div className="container mx-auto px-6 lg:px-12">
              ...
              {/* Results */}
              <AnimatePresence>
                {(visibleResults.length > 0 || loading || normalizedQuery) && (
                  <motion.div
                    ...
                    {loading ? (
                      ...
                      <div className="py-6">
                        <p className="text-sm text-gray-500 mb-4">
                          {visibleResults.length} resultado{visibleResults.length !== 1 ? "s" : ""} encontrado{visibleResults.length !== 1 ? "s" : ""}
                        </p>
                        ...
                        {visibleResults.length > 6 && (
                          <div className="mt-6 text-center">
                            ...
                            >
                              Ver todos os {visibleResults.length} resultados
                            </button>
                          </div>
                        )}
                      </div>
