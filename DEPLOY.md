...
Conferir no App/console InfinityPay:

- handle de produção configurado em `INFINITYPAY_HANDLE`
- API key de produção em `INFINITYPAY_API_KEY`
- webhook apontando para `https://seudominio.com/api/payments/infinitypay/webhook`
- `redirect_url` válido para retorno pós-pagamento

### 11.3. Notificações por e-mail e fallback
...
## 16. Runbook de cutover (feature flag)

1. Deploy com os dois provedores disponíveis (`mercadopago` e `infinitypay`).
2. Validar webhooks dos dois gateways em ambiente de homologação.
3. Em produção, manter `PAYMENT_PROVIDER=mercadopago` e executar smoke:
   - criar pedido
   - finalizar pagamento
   - confirmar atualização de `paymentStatus` e estoque
4. Trocar para `PAYMENT_PROVIDER=infinitypay`.
5. Reiniciar processo (`pm2 restart gtmax3d`) e executar novo smoke.
6. Monitorar por 30-60 minutos:
   - taxa de erro em `/api/payments/checkout`
   - webhook com status 2xx
   - pedidos presos em `PAYMENT_PENDING`
7. Rollback imediato (sem deploy) se houver falha:
   - voltar `PAYMENT_PROVIDER=mercadopago`
   - reiniciar processo
   - revalidar checkout
